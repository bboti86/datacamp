...
bb_count %>%
  ...
  ggplot(aes(y = share, x = chord, fill = chord)) +
  geom_col() +
  coord_flip() + 
...
bb_bigram_count %>%
  ...
  ggplot(aes(y = share, x = bigram, fill = bigram)) +
  geom_col() +
  coord_flip() + 
...
bb_tagged <- bb %>%
  inner_join(tags, by = c("artist" = "artist"))
...
bb_tagged %>%
  ...
  geom_col() +
  facet_grid(vars(instrument)) +
  coord_flip() +
  ylab("Frequnecy of chords") +
  xlab("Chords") 
```
